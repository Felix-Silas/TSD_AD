...
class SignalDataset(Dataset):
    ...
    def make_rolling_signals(self):
        for i in range(-50, 50):
            ...
            self.signal_df['signal' + str(i)] = self.signal_df['signal'].shift(i) # signal 열에서 shift해서 열 창조
...
def train(n_epochs=2000):
    ...
    for epoch in range(n_epochs):
        ...
        logging.debug('Critic training done in epoch {}'.format(epoch + 1))
        encoder_loss = list()
        decoder_loss = list()
        ...
        logging.debug('Encoder decoder training done in epoch {}'.format(epoch + 1))
        logging.debug('critic x loss {:.3f} critic z loss {:.3f} encoder loss {:.3f} decoder loss {:.3f}\n'.format(cx_epoch_loss[-1], cz_epoch_loss[-1], encoder_epoch_loss[-1], decoder_epoch_loss[-1]))
...
if __name__ == "__main__":
    ...
    batch_size = 64
    train_loader = DataLoader(train_dataset, batch_size = batch_size, drop_last = True) # drop_last: 불완전한 배치를 버린다
